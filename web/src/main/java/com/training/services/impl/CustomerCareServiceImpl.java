...
public class CustomerCareServiceImpl implements CustomerCareService {
    ...
    public CustomerCareServiceImpl(UserRepository userRepository, CustomerCareRepository customerCareRepository) {
    ...
    @Override
    public String deleteCustomerCareMessageById(String messageId) {
        if (!customerCareRepository.existsByMessageId(messageId))
            throw new APIException(AppConstants.CUSTOMER_CARE_MESSAGE_NOT_FOUND);

        customerCareRepository.deleteByMessageId(messageId);
